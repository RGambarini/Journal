...
The frequency of longer words that is found in the works of Shakespeare should lead to a higher uncertainty. Or at least that's what we should expect from what we know so far. The longer a word is, the lower the probability of that word should have of occurring, and therefore higher uncertainty. But keep in mind, we're not dealing with a random sequence of words here. Sentences have a predefined structure that goes beyond simple grammatical rules. Sometimes you can predict a statement before it _____. <d-footnote>ends</d-footnote> The same could be said about the structure of words that is not dependent on the use of a dictionary like in our data analysis.
{: style="text-align: justify;"}
...
*For some reason*, the online game [Wordle](https://www.nytimes.com/games/wordle/index.html) became a massive hit in the year 2020 . It challenges a player to guess a five-letter word in six attempts, and reveals feedback on which letters are correct, incorrect, or in the correct position. Naturally, we start by choosing a word that eliminates a significant number of vowels or maybe some very common consonants. With the given feedback, we are able to correctly guess our word in as little as two attempts. <d-footnote>No. No one believes you got it on your first try</d-footnote> This works due to what we've stated before regarding the 'predictability' of the english language. Attempting to play Wordle without any linguistic rules would be quite the challenge. Introducing [Random-le](https://colab.research.google.com/drive/1FgIMvCSQIXEmZ5Ht2muWJ68jDcZjzQBm?usp=sharing), a game that challenges you to guess a random five-character 'string' in six attempts. Not really impossible, but not something i'll be winning or even attempting at all.
{: style="text-align: justify;"}
